...
class TallySupabaseUser extends BaseAuthUser {
  TallySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tallySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TallySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
